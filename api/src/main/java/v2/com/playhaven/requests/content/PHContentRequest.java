...
import v2.com.playhaven.cache.PHImageCache;
...
public class PHContentRequest extends PHAPIRequest implements ContentRequester {
	...
	/** Checks to see if we have dismissed a interstitial view within PREVIOUS_AD_RANGE number of milliseconds.
	 * 
	 ...
	 * @return whether or not a interstitial view was dismissed within the number of milliseconds specified
	 */
	public static boolean didJustShowAd() {
        synchronized (dismissedStamps) {
            ...
            while (curTime - stampTime > PREVIOUS_AD_RANGE && dismissedStamps.size() > 0) {
                stampTime = dismissedStamps.poll();
            }

            // all of the stamps may be too old, let's check
            return (curTime - stampTime <= PREVIOUS_AD_RANGE ? true : false);
        }
	}
